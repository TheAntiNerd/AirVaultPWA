...
export async function POST(request: NextRequest) {
	...
	try {
		...
		if (user_role !== UserRole.OWNER && user_role !== UserRole.ADMIN && user_role !== UserRole.MODERATOR) {
			msg = "You don't have permission to remove groups access from directory!";
			throw new Error(msg);
		}
		...
		const groupName = z.string().min(1).safeParse(requestJson.groupName) ? requestJson.groupName : '';

		// send the request to the server
		const axiosRes = await axiosPost(`${MAIN_URL}/api/acl/permissions/remove_group`, {
			path,
			group: groupName,
		});
