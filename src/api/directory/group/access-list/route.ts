...
export async function POST(request: NextRequest) {
	...
	try {
		...
		if (user_role !== UserRole.OWNER && user_role !== UserRole.ADMIN && user_role !== UserRole.MODERATOR) {
			msg = "You don't have permission see directory access list!";
			throw new Error(msg);
		}
		...
		const axiosRes = await axiosPost(`${MAIN_URL}/api/acl/permissions/groups`, { path });

		if (axiosRes && axiosRes.status === 200 && axiosRes.data.success) {
			permissions = axiosRes?.data?.permissions || {};
			success = true;
			msg = 'Successfully fetched group permissions for directory!';
		} else {
			msg = 'Failed to fetch group permissions for directory!';
		}
	} catch (error) {
		console.error(`An error has occurred while getting group access list. Error -> `, error);
		msg = msg || 'An error has occurred while getting group access list.';
	} finally {
