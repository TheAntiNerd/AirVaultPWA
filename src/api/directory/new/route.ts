...
export async function POST(request: Request) {
	...
	try {
		...
		if (user_role !== UserRole.OWNER && user_role !== UserRole.ADMIN && user_role !== UserRole.MODERATOR) {
			msg = "You don't have permission to create new directory!";
			throw new Error(msg);
		}

		// get the request body
		const requestJson = await request.json();
		const path = z.string().min(1).parse(requestJson.path)?.trim();

		const axiosRes = await axiosPost(`${MAIN_URL}/api/directory/create`, {
			path,
		});
		...
		msg = msg || 'Failed to create directory.';
	} finally {
