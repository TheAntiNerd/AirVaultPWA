...
export default function GroupDelete({ setShowPopup, breadcrumbs, folderName, fetchContents }: Props) {
	...
	async function handleFormSubmit(e: React.FormEvent<HTMLFormElement>) {
		...
		const axiosRes = await axiosPost('/api/directory/delete', {
		...
		<>
			<div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-[110] w-96 rounded-lg">
				<form onSubmit={handleFormSubmit} className="w-full bg-white m-auto rounded-xl p-8">
					<h2 className="text-2xl text-center">Delete directory?</h2>
