...
export default function ClientGroupUser({ setShowPopup, username, path, fetchData }: Props) {
	...
	async function handleFormSubmit(e: React.FormEvent<HTMLFormElement>) {
		...
		const axiosRes = await axiosPost('/api/directory/user/remove', {
			username: username,
			path: path,
		});

		if (axiosRes && axiosRes.status === 200 && axiosRes.data.success) {
			toast.success(axiosRes.data.msg || 'User removed successfully!');
		} else {
			toast.error(axiosRes.data.msg || 'Could not remove user!');
		}
		...
		<>
			<div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-[120] w-96 rounded-lg">
				<form onSubmit={handleFormSubmit} className="w-full bg-white m-auto rounded-xl p-8">
					<h2 className="text-2xl text-center">Remove user?</h2>
