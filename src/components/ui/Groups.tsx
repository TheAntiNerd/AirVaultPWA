				...
				<div className="w-[1200px] max-sm:w-full h-screen pt-10 px-3 max-sm:px-0 bg-white text-sans">
					...
					<div className="overflow-visible max-sm:pb-32 pb-10">
								...
								<thead className="bg-gray-50">
									<tr>
										<div className="max-sm:flex max-sm:justify-between">
											...
											<th className="hidden max-sm:flex max-sm:justify-end py-6 max-sm:pr-16 mr-2 text-sm font-semibold text-gray-600">
												Members
											</th>
										</div>
										...
										<th className="px-6 py-3 text-left text-sm font-semibold text-gray-600 max-sm:hidden"></th>
										<th className="px-6 py-3 text-left text-sm font-semibold text-gray-600 max-sm:hidden"></th>
									</tr>
								</thead>
								...
								<tbody>
									{users.map((user, index) => (
										<tr
											...
											onClick={() => toggleDropdown(index)} // Trigger dropdown on row click
										>
											{/* Desktop Layout */}
											<td className="px-6 py-3 text-[#44475B] max-sm:hidden">
											...
											<td className="px-2 pl-20 py-3 text-gray-600 max-sm:hidden">
											...
											<td
												...
												className="px-6 py-3 text-gray-600 max-sm:hidden">
												<span className="flex flex-col items-center -rotate-90">
													<DownArrow />
												</span>
											</td>
											<td className="px-4 py-3 text-center max-sm:hidden">
												<Dropdown
													...
													<ul className="py-3 text-sm text-left text-[#44475B] absolute right-5 bg-white shadow-2xl rounded-lg w-40 h-24">
														<li
											...
											<td className="hidden max-sm:block">
												...
												<div>
													{activeDropdownIndex === index && (
														<div className="px-3 py-3 text-sm text-[#44475B] font-light">
															<div>
						...
						<div className="max-sm:px-3 max-sm:mb-6 max-sm:fixed max-sm:w-full bottom-6">
							{users.length > 0 && (
								<button
									className={`hidden max-sm:block shadow-custom max-sm:w-full py-3 bg-[#298DFF] text-white rounded-md `}
									onClick={handleAddUserClick}>
									Add Groups
								</button>
							)}
						...
						{users.length === 0 && (
							<div className="flex flex-col items-center justify-center mt-48 text-center max-sm:px-3 max-sm:relative">
								...
								<button
									className={` px-3 shadow-custom max-sm:block max-sm:w-full max-sm:mt-32 py-3 bg-[#298DFF] text-white rounded-md `}
									onClick={handleAddUserClick}>
									Add group
								</button>
							</div>
